# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## References

- When understanding project structure or platform support → read `docs/architecture.md`
- When adding/modifying skills or updating plugin metadata → read `docs/conventions.md`
- When working on agent-benchmark metrics or scoring → read `skills/agent-benchmark/references/metrics.md`
- When working on agent-benchmark task generation → read `skills/agent-benchmark/references/task-templates.md`
- When working on agent-benchmark report output → read `skills/agent-benchmark/references/report-format.md`
- When reviewing agent-benchmark design decisions → read `docs/specs/2026-03-14-agent-benchmark-design.md`
